...
const SITES = ['https://www.mparticle.com/'];
...
for (const site of SITES) {
  describe(site, () => {
    ...
    beforeAll(async () => {
      ...
      await page.goto(site, { waitUntil: 'networkidle2', timeout: 60_000 });
      if (!(await until(20_000, () => captured.length > 0))) {
    ...
    it('emits mParticle traffic that the panel code renders', () => {
      assert.ok(captured.length > 0, `no request matching ${FILTER} on ${site}`);
      assert.ok(captured.some((r) => r.body), `no request body captured on ${site}: ${captured.map((r) => r.url)}`);

      const p = loadPanel();
      for (const r of captured) p.request(entry(r));
      assert.ok(p.$$('#list li').length > 0, `panel rendered no rows for ${captured.length} requests from ${site}`);
    });
    ...
    it('renders events in the real DevTools panel', async () => {
      ...
      assert.ok(rows, `panel showed no rows after reloading ${site}, even after a forced upload`);
      assert.ok(Number(await read(`document.getElementById('count').textContent`)) > 0, 'panel count is not positive');
    });
  });
}
