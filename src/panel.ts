...
const URL_FILTER =
  /\/v[1-3]\/(identify|login|logout|.+\/modify|.+\/config|.+\/Forwarding|JS\/.{4,37}\/events)/i;
...
function probe<T>(
  fn: (mp?: MpInstance) => T | ProbeError,
  cb: (result: T | ProbeError) => void,
) {
  chrome.devtools.inspectedWindow.eval<T | ProbeError>(
    `(${String(fn)})(${instanceExpr()})`,
    (result, err) => {
      if (err)
        return cb({
          error: err.value ? `eval failed: ${err.value}` : "eval failed",
        });
      cb(result);
    },
  );
...
chrome.devtools.network.onNavigated.addListener(() => {
  if (!(document.getElementById("preserve") as HTMLInputElement).checked)
    clear();
  reloadVisible();
});
...
chrome.devtools.network.onRequestFinished.addListener(
  (entry: chrome.devtools.network.Request) => {
    ...
    if (
      (document.getElementById("hide-forwarding") as HTMLInputElement)
        .checked &&
    ...
    if (
      ...
      const { events, ...batch } = body as { events: MpEvent[] } & Record<
        string,
        unknown
      >;
      for (const event of events) {
        const when =
          (event.data && event.data.timestamp_unixtime_ms) ||
    ...
    if (
      typeof body === "object" &&
      body !== null &&
      "known_identities" in body
    ) {
      const req = body as IdentityRequest;
      const { li, meta } = row(
        entry,
        req.request_timestamp_ms || entry.startedDateTime,
      );
      const op = entry.request.url.split("?")[0].split("/").pop();
      const div = summaryEl(meta, {
        event_type: "identity",
        data: { event_name: op },
      });
      ...
      entry.getContent((text) => {
        ...
        if (typeof parsed === "object" && parsed !== null) {
          ...
          if (typeof res.is_logged_in === "boolean") {
            span(
              div,
              res.is_logged_in ? "logged in" : "logged out",
              res.is_logged_in ? "ok" : "muted",
            );
          }
    ...
    if (body === undefined) {
      for (const p of entry.request.queryString || [])
        line(li, `${p.name}=${p.value}`, "param");
    } else {
      raw(li, body);
    }
  },
...
function loadForwarders() {
  ...
  probe(probeForwarders, (result) => {
    ...
    if (!Array.isArray(result)) {
      fwdStatus.textContent =
        (result && (result as ProbeError).error) || "unexpected result";
      return;
    }
    fwdStatus.textContent = result.length
      ? `${result.length} forwarder(s)`
    ...
    for (const f of result) {
      ...
      line(
        meta,
        f.initialized ? "initialized" : "not initialized",
        f.initialized ? "ok" : "err",
      );
...
function loadIdentity() {
  ...
  probe(probeIdentity, (result) => {
    ...
    line(
      meta,
      result.isLoggedIn ? "logged in" : "logged out",
      result.isLoggedIn ? "ok" : "muted",
    );
